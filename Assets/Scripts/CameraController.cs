...
public class CameraController : MonoBehaviour
{
    ...
    private void Update()
    {
        if (hasTarget)
        {
            ...
            Vector3 targetPosition = target.position + ((transform.rotation * -Vector3.forward) * length);
    ...
    private IEnumerator MovePosition(Vector3 target, float time)
    {
        ...
        while (timer < time)
        {
            ...
            transform.position = Vector3.Lerp(start, target, Mathf.SmoothStep(0.0f, 1.0f, timer));
